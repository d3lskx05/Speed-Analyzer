...
def benchmark_model(model_name_or_path, source="HF", n_queries=10, text_length="short", hf_token=None, n_repeats=2):
    ...
    # отключаем dropout, переводим в eval
    try:
        model.eval()
    except Exception:
        pass
...
with col2:
    ...
    if st.button("🗑 Полный сброс сессии"):
        # Очистка session_state и кэшей Streamlit
        st.session_state.clear()
        st.cache_data.clear()
        st.cache_resource.clear()
        try:
            load_model_cached.cache_clear()  # сброс lru_cache
        except Exception:
            pass
        ...
        st.success("Все данные, кэши и ресурсы сброшены. Память очищена.")
